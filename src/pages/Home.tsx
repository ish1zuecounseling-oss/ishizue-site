      ...
      <main id="main-content">
            ...
            <motion.div className="space-y-10">
              ...
              <div className="text-stone-600 leading-relaxed text-base md:text-lg space-y-8">
                ...
                <div className="bg-white p-6 md:p-8 rounded-2xl border border-stone-100 space-y-3">
                  {["役割の過剰化", "責任の抱え込み", "思考のループ", "境界の曖昧さ"].map((item) => (
            ...
            <motion.div className="space-y-12">
              ...
              <div className="bg-stone-50/30 p-6 md:p-16 rounded-3xl border border-stone-100 space-y-10">
                ...
                <div className="text-stone-700 leading-relaxed space-y-8 text-base md:text-lg">
                  <p>15年間、障害福祉の現場で支援者として働いてきました。300名以上、6,000時間を超える支援の中で、ずっと気になっていたことがあります。</p>
                  <p className="text-xl font-medium text-stone-900 border-l-4 border-stone-300 pl-4">支援者自身が、誰にも頼れていない。</p>
                  <p>責任を抱え、感情を押し込め、疲弊しながらも「自分が弱いから」と思い込んでいる人を、何度も見てきました。それは弱さではなく、構造の問題です。</p>
                  <p>支援の軸は、感情の共感だけで終わらせないこと。CBT・ACT・動機づけ面接（MI）・BPSモデル・トラウマインフォームドケアを統合的に用い、思考・役割・負荷を構造的に整理します。「なぜ消耗するのか」を一緒に解きほぐし、持続可能な支援者としての土台を再設計します。</p>
                  <p>個人の努力不足ではなく、構造の問題として捉え直すことが、私の支援の出発点です。</p>
          ...
          <div className="max-w-2xl mx-auto">
            <motion.div className="space-y-12">
              ...
              <form
                ...
                onSubmit={(e) => {
                  e.preventDefault();
                  emailjs.send('service_o133h6q', 'template_va24xdk', {
                    user_name: (e.target as any).name.value,
                    user_email: (e.target as any).email.value,
                    job: (e.target as any).job.value,
                    message: (e.target as any).message.value,
                  }, 'ewvxJYOQmnG4MhwUd')
                  .then(() => { setActiveModal("sent"); })
                  .catch((error) => { console.error(error); alert("送信失敗"); });
                }}
              >
                <div className="space-y-2">
                  <label htmlFor="name" className="text-xs tracking-widest text-stone-500 uppercase">お名前 <span className="text-stone-400">（必須）</span></label>
                  <input id="name" name="name" type="text" required placeholder="山田 花子" className="w-full px-4 py-3 border border-stone-200 rounded-xl text-stone-800 text-sm focus:outline-none focus:ring-2 focus:ring-stone-400 bg-stone-50 placeholder-stone-300" />
                </div>
                <div className="space-y-2">
                  <label htmlFor="email" className="text-xs tracking-widest text-stone-500 uppercase">メールアドレス <span className="text-stone-400">（必須）</span></label>
                  <input id="email" name="email" type="email" required placeholder="example@email.com" className="w-full px-4 py-3 border border-stone-200 rounded-xl text-stone-800 text-sm focus:outline-none focus:ring-2 focus:ring-stone-400 bg-stone-50 placeholder-stone-300" />
                </div>
                <div className="space-y-2">
                  <label htmlFor="job" className="text-xs tracking-widest text-stone-500 uppercase">ご職種</label>
                  <input id="job" name="job" type="text" placeholder="例：社会福祉士、看護師、スクールカウンセラー など" className="w-full px-4 py-3 border border-stone-200 rounded-xl text-stone-800 text-sm focus:outline-none focus:ring-2 focus:ring-stone-400 bg-stone-50 placeholder-stone-300" />
                </div>
                <div className="space-y-2">
                  <label htmlFor="message" className="text-xs tracking-widest text-stone-500 uppercase">ご相談内容 <span className="text-stone-400">（必須）</span></label>
                  <textarea id="message" name="message" required rows={5} placeholder="現在のお困りのこと、ご質問など、お気軽にお書きください。" className="w-full px-4 py-3 border border-stone-200 rounded-xl text-stone-800 text-sm focus:outline-none focus:ring-2 focus:ring-stone-400 bg-stone-50 placeholder-stone-300 resize-none" />
                </div>
                <button type="submit" className="w-full flex items-center justify-center gap-2 px-8 py-4 bg-stone-900 text-stone-50 text-sm tracking-[0.2em] hover:bg-stone-800 transition-all rounded-full shadow-lg">
                  送信する
                </button>
        ...
        <div className="py-6 px-6 border-t border-stone-100">
          <div className="max-w-5xl mx-auto flex flex-wrap justify-center gap-6 text-xs tracking-wider text-stone-400">
            <button onClick={() => setActiveModal("privacy")} className="hover:text-stone-700 transition-colors">プライバシーポリシー</button>
            <button onClick={() => setActiveModal("tokusho")} className="hover:text-stone-700 transition-colors">特定商取引法表記</button>
            <button onClick={() => setActiveModal("cancel")} className="hover:text-stone-700 transition-colors">キャンセルポリシー</button>
          </div>
        </div>
      </main>
      ...
      <AnimatePresence>
        {activeModal && (
          ...
          >
            ...
            >
              <button onClick={() => setActiveModal(null)} className="absolute top-6 right-6 p-2 text-stone-400 hover:text-stone-900 transition-colors" aria-label="閉じる">
                <X className="w-6 h-6" />
              </button>
              ...
              {activeModal === "privacy" && (
                <div className="space-y-8 text-stone-600 leading-loose">
                  ...
                  <section className="space-y-4"><h3 className="text-lg font-medium text-stone-900">1. 取得する情報</h3><p>当相談室では、以下の情報を取得する場合があります。</p><ul className="list-disc pl-5 space-y-1"><li>氏名</li><li>メールアドレス</li><li>相談内容</li><li>予約情報</li><li>決済情報</li></ul></section>
                  <section className="space-y-4"><h3 className="text-lg font-medium text-stone-900">2. 利用目的</h3><p>取得した情報は、以下の目的に利用します。</p><ul className="list-disc pl-5 space-y-1"><li>カウンセリングサービスの提供</li><li>予約管理および連絡対応</li><li>サービス向上のための分析</li><li>法令に基づく対応</li></ul></section>
                  <section className="space-y-4"><h3 className="text-lg font-medium text-stone-900">3. 第三者提供</h3><p>法令に基づく場合を除き、本人の同意なく第三者へ提供することはありません。</p></section>
                  <section className="space-y-4"><h3 className="text-lg font-medium text-stone-900">4. 情報管理</h3><p>取得した情報は適切に管理し、不正アクセス・漏えい・改ざん防止に努めます。</p></section>
                  <section className="space-y-4"><h3 className="text-lg font-medium text-stone-900">5. オンラインツールの利用</h3><p>オンラインカウンセリングでは Google Meet を使用します。各ツールのセキュリティポリシーは提供元の規定に準じます。</p></section>
                  <section className="space-y-4"><h3 className="text-lg font-medium text-stone-900">6. 開示・訂正・削除</h3><p>ご本人からの請求があった場合、法令に従い対応いたします。</p></section>
                  <section className="space-y-4"><h3 className="text-lg font-medium text-stone-900">7. 改定</h3><p>本ポリシーは必要に応じて改定されます。</p></section>
              ...
              {activeModal === "tokusho" && (
                <div className="space-y-8 text-stone-600 leading-loose">
                  ...
                  <div className="grid grid-cols-[120px_1fr] gap-y-6 text-sm md:text-base">
                    <div className="text-stone-400 font-medium">事業者名</div><div>こころの相談室 いしずえ</div>
                    <div className="text-stone-400 font-medium">代表者名</div><div>松本 龍児</div>
                    <div className="text-stone-400 font-medium">所在地</div><div>大阪府大阪市<br /><span className="text-xs text-stone-400">※詳細な所在地については、ご請求があった場合に遅滞なく開示いたします。</span></div>
                    <div className="text-stone-400 font-medium">連絡先</div><div>ish1zue.counseling@gmail.com<br /><span className="text-xs text-stone-400">※お問い合わせはメールにてお願いいたします。</span></div>
                    <div className="text-stone-400 font-medium">販売価格</div>
                    <div className="space-y-2">
                      <p>初回メール相談　無料（1往復）</p>
                      <p>オンラインカウンセリング<br />・初回体験 30分 5,000円（税込）<br />・通常 30分 8,000円（税込）<br />・通常 60分 12,000円（税込）</p>
                      <p>メール相談（通常）<br />・3往復 3,000円（税込）</p>
                    </div>
                    <div className="text-stone-400 font-medium">支払方法</div><div>銀行振込</div>
                    <div className="text-stone-400 font-medium">支払時期</div><div>予約確定後、指定期日までにお支払いください。</div>
                    <div className="text-stone-400 font-medium">提供時期</div><div>入金確認後、予約日時に提供いたします。</div>
                    <div className="text-stone-400 font-medium">返品・返金</div><div>下記キャンセルポリシーをご確認ください。</div>
                  </div>
                </div>
              )}
              ...
              {activeModal === "cancel" && (
                <div className="space-y-8 text-stone-600 leading-loose">
                  ...
                  <ul className="space-y-4">
                    <li className="flex justify-between border-b border-stone-50 pb-2"><span>24時間前までのキャンセル</span><span className="font-medium text-stone-900">無料</span></li>
                    <li className="flex justify-between border-b border-stone-50 pb-2"><span>24時間以内のキャンセル</span><span className="font-medium text-stone-900">料金の50%</span></li>
                    <li className="flex justify-between border-b border-stone-50 pb-2"><span>当日・無断キャンセル</span><span className="font-medium text-stone-900">料金の100%</span></li>
                  </ul>
              ...
              {activeModal === "sent" && (
                <div className="text-center space-y-6 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-stone-500 text-sm leading-relaxed">お問い合わせありがとうございます。<br />通常2営業日以内にご返信いたします。</p>
                  </div>
                  <button onClick={() => setActiveModal(null)} className="inline-flex items-center px-8 py-3 bg-stone-900 text-stone-50 text-sm tracking-[0.15em] hover:bg-stone-800 transition-all rounded-full">
                    閉じる
                  </button>
                </div>
              )}
