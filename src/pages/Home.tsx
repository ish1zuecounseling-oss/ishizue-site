...
// Fix D: 声を「具体的に何が変わったか」を含む内容に書き直し
const voices = [
  {
...
// Fix G: 3ステップを「読者の不安を取り除く言葉」に書き直し
const flowItems: FlowItem[] = [
  {
    step:  "01",
    title: "フォームを送る（2分）",
    desc:  "今の状態を書くだけでOK。まとまっていなくて大丈夫です。",
    note:  "送った後も、キャンセル可能です",
  },
  {
    step:  "02",
    title: "メールが届く（2営業日以内）",
    desc:  "状況を確認しながら、次のステップをご案内します。",
    note:  "押し売りなし。断るのも自由です",
  },
  {
    step:  "03",
    title: "事前シート記入（任意）",
    desc:  "簡単な状況把握シートにご記入いただきます。任意項目あり。",
    note:  "書けない部分はそのままで大丈夫",
  },
  {
    step:  "04",
    title: "体験セッション 30分",
    desc:  "Google Meetにて。この時間だけで「何が負担か」が整理されます。",
    note:  "続けるかどうかは、その後に決めればいい",
  },
  {
    step:  "05",
    title: "振り返りシート共有",
    desc:  "セッション後、整理内容をテキストでお送りします。手元に残ります。",
    note:  "1回で終わっても問題ありません",
  },
];
  ...
  return (
    <>
      ...
      <main id="main-content">
        ...
        {/* ================================================================
            HERO
            Fix A: 「問いかけ→着地」構造に変更。最後に「ここはあなたのための場所」と言い切る
        ================================================================ */}
          ...
          <div className="relative max-w-5xl mx-auto px-5 md:px-8 py-24 md:py-40 w-full">
            <motion.div initial="hidden" animate="visible" variants={stagger} className="space-y-6 md:space-y-8">
              ...
              <motion.div variants={fadeUp} className="space-y-4 max-w-2xl">
                ...
                {/* Fix A: 問いかけ→共感→「ここはあなたのための場所」と着地させる */}
        ...
        {/* ================================================================
            PAIN POINTS
            Fix B: タイトルを「即座に自分のことだと気づける」1行に変更
        ================================================================ */}
          ...
          <div className="max-w-4xl mx-auto space-y-10 md:space-y-14">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={stagger}>
              <motion.div variants={fadeUp} className="space-y-3 max-w-xl">
                ...
                {/* Fix B: 「あなたの話ではないですか」→ 読者が即座に自分と重ねる */}
              ...
              <motion.div variants={fadeUp}>
                <div className="flex flex-col md:grid md:grid-cols-[180px_1fr] gap-8 md:gap-10 items-start">
                  ...
                  <div className="text-stone-600 leading-[2.1] text-sm md:text-base space-y-6" style={{ fontFamily: "'Noto Serif JP', serif" }}>
                    ...
                    {/* Fix C: 申し込み後の未来像を明示 */}
                    <div className="pt-5 border-t border-stone-200 p-5 rounded-2xl bg-stone-50">
                  ...
                  {[
                    ...
                    <div key={ex.case} className="bg-stone-50 rounded-2xl border border-stone-100 p-5 space-y-2.5 text-sm">
                      ...
                      <div className="text-stone-500 text-xs leading-relaxed pl-3 border-l border-stone-200">
                        <p>{ex.after}</p>
                        <p className="mt-1">→ 構造として整理することで、対処が見えてくる</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {/* ================================================================
            VOICES
            Fix D: before/after 構造で「具体的に何が変わったか」を明示
        ================================================================ */}
                  ...
                  <motion.div key={v.role} variants={fadeUp} className="bg-white border border-stone-100 rounded-2xl p-5 space-y-4">
                    ...
                    <div className="border-t border-stone-100 pt-3">
                      <div className="text-[#8FAF9F] opacity-50 mb-2">
                        <svg width="20" height="15" viewBox="0 0 24 18" fill="currentColor">
        ...
        {/* ================================================================
            CLOSING — Fix J: 短い「一言クロージング」セクション
            Voicesの余韻のまま、一息ついて申し込みへ促す
        ================================================================ */}
        <section className="py-14 px-5 bg-white border-y border-stone-100">
          ...
          >
            <p className="text-stone-800 text-lg md:text-2xl font-light leading-[1.9]" style={{ fontFamily: "'Noto Serif JP', serif" }}>
              支援を続けるために、<br />
              <span className="text-[#8FAF9F]">一度、自分のために使う時間があっていい。</span>
            </p>
            <p className="text-stone-500 text-sm">
              それは弱さではなく、支援を続けるための判断です。
            </p>
          </motion.div>
        </section>
        ...
        <section id="guide" className="py-16 md:py-24 px-5 md:px-6 bg-stone-950">
          <div className="max-w-4xl mx-auto">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={stagger} className="space-y-14 md:space-y-20">
              ...
              {/* 流れ — Fix G: noteフィールドで「不安を取り除く一言」を添える */}
              <motion.div variants={fadeUp} className="space-y-6">
                  ...
                  {flowItems.map((item) => (
                    <li key={item.step} className="flex gap-5 items-start group">
                      ...
                      <div className="flex-1 py-4 border-b border-stone-900 group-last:border-b-0">
                        ...
                        {/* Fix G: 不安を取り除くnoteを追加 */}
                        <p className="text-[#8FAF9F]/70 text-[11px] mt-1 italic">{item.note}</p>
                      </div>
                    </li>
                  ))}
                </ol>
