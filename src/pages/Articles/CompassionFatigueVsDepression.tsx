...
export default function CompassionFatigueVsDepression() {
  return (
    ...
    >
      ...
      <h2>なぜ混同されやすいのか：定義と心理的メカニズム</h2>
      <p>
        共感疲労（Compassion Fatigue）とは、支援者が利用者・患者・相談者の苦しみに継続的に共感することで生じる、
        心理的・感情的・身体的な消耗状態のことです。侵入症状（利用者の話が頭から離れない）・回避症状（特定のケースを
        避けたくなる）・過覚醒（常に緊張している）・感情の麻痺が主な症状です。重要な特徴は、仕事から離れると
        症状が緩和される傾向がある点です。
      </p>
      <p>
        うつ（抑うつ状態）は、持続的な抑うつ気分・興味と喜びの喪失・疲労感・自己否定感などを特徴とする状態です。
        仕事以外の生活全般にわたって影響が及ぶ点が重要な特徴です。
      </p>
      <p>
        疲労感・睡眠障害・集中力の低下という症状が共通して現れること、消耗した状態では自己評価が難しくなること、
        支援職は自分の消耗を「この程度で」として過小評価しやすいことが、混同を生みやすくしています。
      </p>

      <h2>現場で起こる具体例：「どちらかわからない」が生まれる場面</h2>
      <h3>■ 相談支援の現場：仕事を離れると少し楽になる</h3>
      <p>
        担当ケースのことが頭から離れず、夜も眠れない日が続く。しかし長期休暇中に仕事のことを考えない時間が増えると、
        少し気持ちが楽になる感覚がある。「仕事から離れると楽になる気がする」という体験は、共感疲労の側面が
        強い可能性があります。
      </p>
      <h3>■ 福祉職の現場：休日も趣味が楽しめない</h3>
      <p>
        仕事のことを考えていないはずの時間でも気持ちが晴れない。「何もしていない自分がダメな気がする」という
        強い自己否定が続く。仕事以外の生活全般への影響と強い自己否定感は、うつのサインとして注意が必要です。
      </p>
      <h3>■ 心理職の現場：クライアントへの共感が急に戻る体験</h3>
      <p>
        長期休暇に入った直後から学校にいたときの緊張が緩み、数日後には「また生徒たちに関わりたい」という気持ちが
        戻ってきた。休暇中に症状が改善した体験は、共感疲労からの一時的な回復として理解できます。
      </p>
      <h3>■ 就労支援の現場：両方が重なっている状態</h3>
      <p>
        担当利用者の状況が頭から離れず（共感疲労的な側面）、かつ仕事を離れた休日にも気分が晴れず自分を責め続ける
        （うつ的な側面）状態が続く。実際の現場では両方が重なって現れることは珍しくなく、専門家への相談が
        重要になります。
      </p>

      <h2>関連概念とのつながり：共感疲労・うつとバーンアウトの関係</h2>
      <p>
        共感疲労・うつ・バーンアウト（燃え尽き症候群）はそれぞれ異なる概念ですが、深く絡み合っています。
        バーンアウトは主に仕事という文脈での消耗であり、仕事から離れると改善する傾向がある点で共感疲労と
        似た特性を持ちます。共感疲労が放置されるとバーンアウトへ移行し、バーンアウトが放置されるとうつへ移行する
        リスクが高まるという段階的な深刻化が起きることがあります。
      </p>
      <p>
        感情労働の蓄積は共感疲労とうつの両方のリスクを高めます。感情を抑制し続けることで感情の自己疎外が生じ、
        これが両者の区別をさらに難しくします。共感疲労の段階では適切なバウンダリーの維持と多層的な休息が回復の鍵に
        なり、うつの段階では医療的なサポートが不可欠になることが多くあります。
      </p>

      <h2>実践的な整理：明日からできる3つの視点</h2>
      <h3>① 「仕事を離れると楽になるか」を確認する</h3>
      <p>
        休日・休暇中に症状が緩和される感覚があれば、共感疲労の側面が強い可能性があります。仕事以外の場面でも
        気分の重さが続き、好きなことが楽しめない状態が2週間以上続く場合は、うつのサインとして専門家への
        相談を検討することが助けになります。
      </p>
      <h3>② 「自己否定の強さ」を観察する</h3>
      <p>
        共感疲労では「仕事の対応への不全感」が中心になりやすい一方、うつでは「自分という存在への否定」が中心に
        なりやすい傾向があります。「自分はダメな人間だ」「消えてしまいたい」という感覚が続いている場合は、
        医療的な相談を優先することが重要です。
      </p>
      <h3>③ 「判断しない」という選択を意識的に持つ</h3>
      <p>
        消耗した状態では「これは共感疲労か、うつか」を自分一人で判断しようとすることに限界があります。
        主治医・心療内科・精神科・カウンセラーへの相談を、「自分で判断できないから相談する」という形で
        活用することが助けになります。
      </p>

      <h2>まとめ</h2>
      <ul className="space-y-2">
        <li>影響の範囲：共感疲労は仕事・支援という文脈に結びつきやすく、うつは生活全般に及ぶ</li>
        <li>仕事から離れたときの変化：共感疲労は仕事から離れると改善しやすく、うつは仕事以外の場面でも続く</li>
        <li>自己否定の質と強さ：共感疲労は仕事への不全感、うつは自己存在への否定が中心になりやすい</li>
